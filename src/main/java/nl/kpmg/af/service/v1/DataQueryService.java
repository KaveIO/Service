 ...
 *
 * Usage:
 * $ curl {https, user, password, host, prefix if on JBoss}/v1/dataQuery/{applicationId}/{collection}
 *        -d '{"foo": "bar"}' -H "Content-Type: text/plain"
 *
 * We aim to keep the queries generic, as in the above "foo": "bar", so without MongoDB-specific operators $gt, $lt etc.
 * to avoid tightly coupling any clients to the MongoDB query language.
 *
 * @author jvlier
 */
@Service
...
public class DataQueryService {
     ...
     *
     * This service currently does *not* use paging, because it's not needed for the use case for which it was
     * developed initially (LAS requests on aggregated data, around 1 to 100 documents maximum in typical queries).
     * So it didn't seem worthwhile to burden this service with the additional complexity of pagination.
     * Feel free to add it if needed though.
     *
    ...
    public Response getData(
        ...
        try {
            ...
            return Response.ok(new MeasurementsRepresentation(content, new HashMap<String, Link>())).build();
