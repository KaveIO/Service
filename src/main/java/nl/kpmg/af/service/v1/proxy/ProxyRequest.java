...
public class ProxyRequest {
      ...
      if (proxy.getMethodsAllowed().contains(request.getMethod().getMethodString())) {
        ...
        try {
          ...
          return Response.status(connection.getResponseCode())
              .header("Access-Control-Allow-Origin", "*").entity(connection.getInputStream())
              .build();
        } catch (MalformedURLException ex) {
          LOGGER.error("Proxy defintion contains malformed URL", ex);
          return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
        } catch (IOException ex) {
          if (!proxy.isDisableSSL()
              && CertificateException.class.isAssignableFrom(ex.getCause().getClass())) {
            LOGGER.error(
                "Call couldn't be made to proxied service - proxy target possibly has an unsigned "
                    + "certificate, this is avoidable by setting disable_ssl on the proxy object.",
                ex);
          } else {
  ...
  /**
   * If an username is set on the proxy object this will be used to Authenticate against the
   * backend.
   *
   * @param connection
   */
  private void connectionAuthorize(HttpURLConnection connection) {
    if (proxy.getUsername() != null) {
      String encoded = DatatypeConverter
          .printBase64Binary((proxy.getUsername() + ":" + proxy.getPassword()).getBytes());
  ...
  private void connectionSendContent(HttpURLConnection connection) throws IOException {
    if (request.getContentLength() > 0) {
      ...
      /*
       * We have to set the Content-Type to something as silly as * / * if we have no content to
       * send. By default text/html is assumed by HttpURLConnection. This will break rest backends
       * that don't expect any input.
       */
  ...
  public static boolean isRecursiveCall(String request, String proxy) {
    try {
      ...
      if (requestUrl.getHost().equals(targetUrl.getHost())
          ...
          && requestUrl.getPort() == targetUrl.getPort() && pattern.matcher(proxy).matches()) {
        LOGGER.info(
            "The proxy target is another call to the proxy server. To avoid recursive call this is not permitted");
        return true;
      }
